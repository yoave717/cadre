...
describe('Edit Tools', () => {
    ...
    describe('editFile', () => {
        it('should fail if file has not been read', async () => {
            files.hasBeenRead.mockReturnValue(false);
        ...
        it('should replace a string successfully', async () => {
            files.hasBeenRead.mockReturnValue(true);
            fs.readFile.mockResolvedValue('Hello old world');
            fs.writeFile.mockResolvedValue(undefined);
            const result = await edit.editFile('test.txt', 'old', 'new');
            expect(fs.writeFile).toHaveBeenCalledWith(path.resolve('test.txt'), 'Hello new world', 'utf-8');
        ...
        it('should fail if old string is ambiguous and replaceAll is false', async () => {
            files.hasBeenRead.mockReturnValue(true);
            fs.readFile.mockResolvedValue('Hello old old world');
            const result = await edit.editFile('test.txt', 'old', 'new');
        ...
        it('should replace all occurrences if replaceAll is true', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(path.resolve('test.txt'), 'Hello new new world', 'utf-8');
        });
    });
