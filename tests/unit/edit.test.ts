...
describe('Edit Tools', () => {
  ...
  describe('editFile', () => {
    it('should fail if file has not been read', async () => {
      (files.hasBeenRead as any).mockReturnValue(false);
    ...
    it('should replace a string successfully', async () => {
      (files.hasBeenRead as any).mockReturnValue(true);
      (fs.readFile as any).mockResolvedValue('Hello old world');
    ...
    it('should fail if old string is ambiguous and replaceAll is false', async () => {
      (files.hasBeenRead as any).mockReturnValue(true);
      (fs.readFile as any).mockResolvedValue('Hello old old world');
    ...
    it('should replace all occurrences if replaceAll is true', async () => {
      ...
      const result = await edit.editFile('test.txt', 'old', 'new', true);
  ...
  describe('insertAtLine', () => {
      it('should insert content at specific line', async () => {
          (files.hasBeenRead as any).mockReturnValue(true);
          (fs.readFile as any).mockResolvedValue('line1\nline2\nline3');
